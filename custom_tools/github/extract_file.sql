/*
===============================================================================
GITHUB FILE EXTRACTOR - GraphQL API Version
===============================================================================

PURPOSE:
Fetches file contents from GitHub repositories using the GraphQL API.
Retrieves text files and provides metadata about file size and type.
Essential for accessing complete SQL queries when they're truncated in 
Snowflake's query history.

KEY FEATURES:
- Uses GitHub GraphQL API for efficient content retrieval
- Returns file content as text with metadata
- Handles authentication and rate limiting gracefully
- Detects and reports binary files (cannot retrieve content)
- Provides direct GitHub URL for reference

USAGE:
- GITHUB_FILE_EXTRACTOR: Fetch a specific file from a repository
...
- [SECTION_1_PROCEDURE]    : Main stored procedure definition
- [SECTION_2_PERMISSIONS]  : Grant statements
- [SECTION_3_EXAMPLES]     : Example usage scenarios
- [SECTION_4_TIPS]         : Usage tips and best practices

PREREQUISITES:
- GitHub integration must be set up first
- Run setup_github_integration.sql before creating this procedure

===============================================================================
*/

-- [SECTION_1_PROCEDURE] Main Stored Procedure
-- ===========================================
USE ROLE SYSADMIN;
USE DATABASE SNOWFLAKE_INTELLIGENCE;
USE SCHEMA TOOLS;

-- Drop the old procedure first to avoid signature conflicts
DROP PROCEDURE IF EXISTS GITHUB_FILE_EXTRACTOR(VARCHAR, VARCHAR, VARCHAR, VARCHAR, VARCHAR);
